...
//- (void) setCoordinates:(void (^)(NSError * _Nullable error))completion
//{
//    self.locationManager = [[CLLocationManager alloc] init];
//    self.locationManager.delegate = self;
//    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0"))
...
//- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations withCompletion:(void (^)(NSError * _Nullable error))completion
//{
//    self.restaurantCoordinates = [[Coordinate shared] setCoordinateValuesWithLatitude:[[manager location] coordinate].latitude andLongitude: [[manager location] coordinate].longitude];
//    NSMutableArray *placeholder = [[NSMutableArray alloc] init];
//    if (!self.competitorArray){
...
//        PFUser *currentUser = [PFUser currentUser];
//        self.userParameters = [[NSMutableArray alloc] initWithObjects:currentUser[@"address"], currentUser[@"category"], currentUser[@"Price"], nil];
//        [self locationTopRatings:self.userParameters[1] withPrice:self.userParameters[2] withIndex:0 withCompletion:^(NSError * _Nullable error) {
//            if (error){
//                completion(error);
//            } else {
//                completion(nil);
...
//}
//
//-(void)locationTopRatings:(nullable NSString *)categoryRes withPrice:(nullable NSString *)priceRes withIndex:(NSUInteger)index withCompletion:(void (^)(NSError * _Nullable error))completion{
//
//    NSDictionary *headers = @{
//                              @"Authorization": @"Bearer Z505A_B9SNUBRJYRkioQ9NX8ZD9AnREWx3MqrxHSny1dop_ox6v0Ptx2-qbqX6fktt79CfqzXYdCcc6j3iE6BMTK6QHsDThNMbPYSf1mWXec1p7zsC6MupJVmkU2XXYx",
//                              @"User-Agent": @"PostmanRuntime/7.15.0",
